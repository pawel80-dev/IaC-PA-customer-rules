...
def create_config(file_path, sheet_name=None) -> None:
    ...
    for row in excel_sheet.iter_rows(min_row=1, max_col=excel_sheet.max_column, max_row=excel_sheet.max_row):
        ...
        if row[0].value == "Policies":
            config_flag = "policy"
        if (row[0].value is not None and row[0].value != "Group_address_objects" 
            and row[0].value != "Group_name" and config_flag == "addr_group"):
            # set the key and use it for the next addresses objects values
            key = row[0].value
            cfg_addr_groups[key] = {
                row[1].value: row[2].value,
            }
        if row[0].value is None and row[1].value is not None and config_flag == "addr_group":
            cfg_addr_groups[key][row[1].value] = row[2].value

        # flag was changed, time for a second part of the config
        if (row[0].value != "Policies" and row[0].value != "Policy_name" 
            and config_flag == "policy" and row[7].value != "application-default" and row[7].value != "any"):
